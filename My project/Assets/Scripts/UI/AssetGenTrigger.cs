...
namespace AIBeat.Core
{
    public static class AssetGenTrigger
    {
        ...
        public static void GenerateAndSaveAssets()
        {
            ...
            string uiDecoPath = Path.Combine(uiPath, "Decorations");
            if (!Directory.Exists(uiDecoPath)) Directory.CreateDirectory(uiDecoPath);

            var drum = ProceduralImageGenerator.CreateInstrumentTexture("Drum");
            ProceduralImageGenerator.SaveTextureAsPNG(drum, Path.Combine(uiDecoPath, "Drum_Silhouette.png"));

            var piano = ProceduralImageGenerator.CreateInstrumentTexture("Piano");
            ProceduralImageGenerator.SaveTextureAsPNG(piano, Path.Combine(uiDecoPath, "Piano_Silhouette.png"));

            var guitar = ProceduralImageGenerator.CreateInstrumentTexture("Guitar");
            ProceduralImageGenerator.SaveTextureAsPNG(guitar, Path.Combine(uiDecoPath, "Guitar_Silhouette.png"));
            
            var notes = ProceduralImageGenerator.CreateInstrumentTexture("Notes");
            ProceduralImageGenerator.SaveTextureAsPNG(notes, Path.Combine(uiDecoPath, "MusicNotes_Icons.png"));

            // ---------------------------------------------------------
            // EXPORT FOR CLAUDE (Assets/Sprites/Instruments/)
            // ---------------------------------------------------------
            string spritePath = Path.Combine(Application.dataPath, "Sprites/Instruments");
            if (!Directory.Exists(spritePath)) Directory.CreateDirectory(spritePath);

            ProceduralImageGenerator.SaveTextureAsPNG(drum, Path.Combine(spritePath, "drum_silhouette.png"));
            ProceduralImageGenerator.SaveTextureAsPNG(piano, Path.Combine(spritePath, "piano_silhouette.png"));
